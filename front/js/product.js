...
function getOneProductId() {
  ...
  fetch(`http://localhost:3000/api/products/${oneProductId}`)
    ...
    .catch((err) => {
      // on affiche une erreur qui modifie la page produit si l'api ne réponds pas
...
function displayProduct(product) {
  ...
  for (let color of product.colors) {
    // Et création d'une boucle pour l'affichage des differentes couleurs renseignées dans l'api de notre produit => "product.colors"
...
function getActualProductInLocalStorage() {
  // fonction pour recuperer des produits du panier "cart" déjà existants dans le local storage, s'il n'y en a pas, on aura un tableau vide
...
async function addToLocalStorage() {
  ...
  const productItemData = {
    // on met les données du produit dans un array pour l'affichage dans le local storage
    id: product._id,
  ...
  if ((quantityImput.value < 1 || quantityImput.value > 100) && colorSelect.value == "") {
    // on veut afficher une fenetre d'alerte si la couleur et/ou la quantité n'est pas renseignée
    ...
    if (!productChoiceFind) {
      // si ce produit n'est pas déja dans le panier alors on l'ajoute.
      productChoice.push(productItemData);
    } else if (productChoiceFind) {
      // si il est déjà présent avec la meme couleur alors on ajoute seulement sa quantité
      productChoiceFind.quantity += parseInt(quantityImput.value);
    } else if (productChoiceFind && productChoiceFind.color != colorSelect.value) {
      // si il est déjà présent mais d'une couleur différente alors on l'ajoute
      productChoice.push(productItemData);
    }
    saveProductToLocalStorage(productChoice);
    // on affiche une boite de dialogue pour rediriger l'utilisateur sur le panier ou continuer ses achats
    if (
      confirm(
        `Le canapé ${product.name} ${colorSelect.value} a été ajouté au panier en ${quantityImput.value} exemplaire(s)\n 
Passez commande en cliquant sur OK ou continuez vos achats`
      )
    ) {
      location.href = `/front/html/cart.html`;
    }
  }
}
